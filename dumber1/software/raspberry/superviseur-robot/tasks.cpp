...
void Tasks::Init() {
    ...
    if (err = rt_mutex_create(&mutex_robotStarted, NULL)) {
     ...
     if (err = rt_mutex_create(&mutex_withWatchdog, NULL)) {
        cerr << "Error mutex create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
    ...
    if ((err = rt_queue_create(&q_messageToMon, "q_messageToMon", sizeof (Message*)*100, Q_UNLIMITED, Q_FIFO)) < 0) {
     ...
     if ((err = rt_queue_create(&q_messageToControlComRobot, "q_messageToControlComRobot", sizeof (Message*)*500, Q_UNLIMITED, Q_FIFO)) < 0) {
        cerr << "Error msg queue create: " << strerror(-err) << endl << flush;
        exit(EXIT_FAILURE);
    }
...
void Tasks::StartRobotTask(void *arg) {
    ...
    while (1) {
        ...
        /*Recuperer la valeur qui definira si la Com sera avec ou sans watchdog*/
        rt_mutex_acquire(&mutex_move, TM_INFINITE);
        rs =withWatchdog;
        rt_mutex_release(&mutex_move);
...
void Tasks::MoveTask(void *arg) {
    ...
    while (1) {
        ...
        cout << "Periodic mouvement update";
        rt_mutex_acquire(&mutex_robotStarted, TM_INFINITE);
        rs = robotStarted;
        rt_mutex_release(&mutex_robotStarted);
        if (rs == 1) {
...
void Tasks::BatterylevelTask(void *arg) {
    int rs;
    MessageID battery =  MESSAGE_ROBOT_BATTERY_GET;
    //MessageID battery = "Robot battery level";
    ...
    while(1){
         ...
         rt_task_wait_period(NULL);
         cout << "Periodic Batterylevel update";
         //verifier que le robot est actif
         rt_mutex_acquire(&mutex_robotStarted, TM_INFINITE);
         rs = robotStarted;
         rt_mutex_release(&mutex_robotStarted);
        
         if(rs == 1){
            cout << " start level battery " << endl << flush;
            //demander et recuperer le niveau de la batery
            rt_mutex_acquire(&mutex_robot, TM_INFINITE);
